analysis.addSelectionGUI = function() {
    let guis = ["Higgs", "W", "Z"];
    analysis.guis = guis.map(tlt => {
        let gui = new dat.GUI({
            name: tlt,
            hideable: false,
            autoPlace: false
            });
        gui.domElement.id = tlt.toLowerCase() + "GUI";
        document.getElementById(tlt.toLowerCase() + "-tab").appendChild(gui.domElement);
        return gui;
    });
	nMuon = -1;
	nElectron = -1;
    ...
    analysis.selectionFields = {};

    analysis.guis.forEach(gui => {
        gui.add({"Plot": false}, 'Plot')
        Object.keys(row_obj).forEach(function(key) {
            elem_name = naming_map[key];
    
            // add the controller to the folder
            if (key == "charge") {
                var cont = gui.add(row_obj, key, ["", "same", "opposite"]).name(elem_name);
